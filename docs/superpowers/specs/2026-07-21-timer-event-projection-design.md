...
The event stream is the communication mechanism. Projections are the reactive state read by the UI. Components publish intent and render state; they do not coordinate persistence, generation, device leasing, or sibling updates.
...
1. Components publish typed events through small event-family emitters.
2. Components never build event envelopes by hand.
3. Request events express intent. Result/fact events are the only events that update projections.
4. Service modules consume requests, perform work, and publish success or failure events.
5. Projection modules consume facts/results and synchronously update reactive state.
6. Every Timer projection filters owner-scoped events by `ownerId` and generation events by `scopeId`.
7. Programmatic events use the application monotonic clock. Browser/device events preserve their native timestamp.
8. High-frequency elapsed-time readings continue through the direct callback channel and do not enter the EventBus.
9. All module attachment returns an idempotent detach operation.
10. Detached modules must stop reacting, including after Timer route remounts.
11. Expected failures become typed failure events; they do not escape into Svelte components.
12. The migration preserves current behavior and follows the existing Figma visual design. It does not add Timer features.
...
- stable bus and event-emitter infrastructure;
- readonly projections;
- the owner ID;
- readiness and teardown.

It must not become a large command facade. Existing methods such as `requestSolveUpdate` are migration bridges and should be replaced at component boundaries by event-family emitters.
...
- supplying narrow readonly projections and emitter infrastructure.
...
Renders lifecycle, scramble, preview, and active-device projections. Native input boundaries emit events. Approved platform integration such as preventing sleep belongs behind a small boundary module, not a direct general-purpose data-service dependency.

### `TimerOptions.svelte`

Matches the Figma options experience and emits requests for scramble refresh, provided/previous scramble use, session setting changes, active-device changes, and other existing controls. It does not call `sessionController`, `dataService`, or an `initInputHandler` callback.

### `HistoryTab.svelte`

Reads the solve projection and emits list, update, and remove requests. Modal open state, pagination, filtering, and temporary editing state may remain local. Preview generation uses scoped image events.

### `StatsTab.svelte`

Reads the statistics projection. Chart instances and presentation-only interaction remain local to the component.
...
3. Expose a narrow readonly Timer view and event-family emitters to components.
...
- UI components render projection error state or trigger the existing notification boundary.
...
- Timer UI components emit intent rather than importing forbidden services/controllers.
...
- all Timer UI communication follows typed event publication and readonly projections;

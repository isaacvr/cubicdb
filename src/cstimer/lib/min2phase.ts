...
let USE_CONJ_PRUN = USE_TWIST_FLIP_PRUN;
...
let ud2std = [Ux1, Ux2, Ux3, Rx2, Fx2, Dx1, Dx2, Dx3, Lx2, Bx2, Rx1, Rx3, Fx1, Fx3, Lx1, Lx3, Bx1, Bx3];
let std2ud = [];
let ckmv2bit = [];
let urfMove = [
...
let MPermMove = [];
let MPermConj = [];
let CCombPMove = []; // = new char[N_COMB][N_MOVES2];
let CCombPConj = [];
let MCPermPrun = [];
let EPermCCombPPrun = [];

let TwistFlipPrunMax = 15;
let UDSliceTwistPrunMax = 15;
...
function initRawSymPrun(PrunTable, N_RAW, N_SYM, RawMove, RawConj, SymMove, SymState, PrunFlag) {
  ...
  let SEARCH_DEPTH = PARTIAL_INIT_LEVEL > 0 ?
  ...
  while (depth < SEARCH_DEPTH) {
    ...
    for (let i = 0; i < N_SIZE; i++, val >>= 4) {
      ...
      for (let m = 0; m < N_MOVES; m++) {
        ...
        for (let j = 1, symState = SymState[symx];
          (symState >>= 1) != 0; j++) {
          if ((symState & 1) != 1) {
            continue;
          }

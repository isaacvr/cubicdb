<script lang="ts">
  ...
  function findIcon(ic: string): Scrambler | null {
...
<button type="button" { tabindex } class={"rounded-md p-1 grid place-items-center w-fit cursor-default " + buttonClass}
  class:selected on:click={ handleClick }>
  {#if findIcon(icon) }
    <svelte:component this={ iconMap[findIcon(icon) || '333'] } {size} {width} {height} {color} {ariaLabel} {ariaHidden} {title} {desc} class={cl}/>
  {:else if !noFallback}

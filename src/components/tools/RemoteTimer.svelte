<script lang="ts">
  ...
  let solveControl = [
    { text: "Delete", icon: Close, highlight: (p: any) => false, handler: () => {
      machine.delete();
    }},
    { text: "DNF", icon: ThumbDown, highlight: (p: any) => penalty === Penalty.DNF, handler: () => {
      penalty = penalty === Penalty.DNF ? Penalty.NONE : Penalty.DNF;
      machine.penalty(penalty);
    } },
    { text: "+2", icon: Flag, highlight: (p: any) => penalty === Penalty.P2, handler: () => {
      penalty = penalty === Penalty.P2 ? Penalty.NONE : Penalty.P2;
      machine.penalty(penalty);
    } },
    { text: "Comments", icon: CommentIcon, highlight: (p: any) => false, handler: () => {
      machine.editSolve();
    } }
  ];
  ...
  function getColor(st: typeof $mState): string {
    ...
    switch(st) {
      case 'CLEAN': { col = 'gray'; break; }
      
      case 'CONNECTING':
      case 'STARTING': { col = 'yellow'; break; }
      
      case 'CONNECTION_ERROR':
      case 'DISCONNECTED': { col = 'red'; break; }
      
      case 'READY': { col = 'green'; break; }
      case 'RUNNING': { col = 'primary'; break; }
      case 'STOPPED': { col = 'purple'; break; }
...
<div class="bg-backgroundLv1 w-[min(90%,36rem)] flex mx-auto my-4 p-2 border-l-4 border-l-primary-700 landscape:hidden">
...
<Card class="flex flex-row relative items-center justify-between mt-4 w-[calc(100vw-2rem)]
  max-w-[unset] mx-auto mb-8" style="aspect-ratio: 3 / 1 !important;">
  
  <Card class={"h-full aspect-square border !border-primary-700 shadow-primary-700 !p-0 " +
    ($mLeftDown ? '!bg-primary-800 ' : '')}>

    <button class={"w-full h-full grid place-items-center "}
      class:pressed={ $mLeftDown } on:touchstart|preventDefault
      on:pointerdown={handleLeft} on:pointerup={handleLeft}>
  ...
  <div class="h-full w-full grid">
    ...
    <div class={"flex justify-evenly w-full items-end " + ($mState === 'STOPPED' ? '' : 'hidden')}>
      {#each solveControl as control}
        <Button color="none" class="flex my-3 mx-1 w-5 h-5 p-0 { control.highlight(penalty) ? 'text-red-500' : '' }"
    ...
    <div class="flex justify-evenly items-center mt-auto">
      {#if $mState != 'DISCONNECTED' && $mState != 'CONNECTING' && $mState != 'CONNECTION_ERROR'}
        <Button color="none" class="rounded-full border border-red-500 ring-red-800 p-3" on:click={ onOff }>
  ...
  <Card class={"h-full aspect-square border !border-primary-700 shadow-primary-700 !p-0 " +
    ...
    <button class={"w-full h-full grid place-items-center "}
      class:pressed={ $mRightDown } on:touchstart|preventDefault
      on:pointerdown={handleRight} on:pointerup={handleRight}>

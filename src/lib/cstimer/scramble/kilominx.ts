...
const perm4Mult: number[][] = [];
const perm4MulT: number[][] = [];
const perm4TT: number[][] = [];
...
const Phase1Move: any[] = [];
const Phase2Move: any[] = [];
const Phase3Move: any[] = [];
const Phase1Prun: number[] = [];
const Phase2Prun: number[] = [];
const Phase3Prun: number[] = [];
...
function move2str(moves: number[][]) {
  ...
  for (let i = 0; i < moves.length; i++) {
    ret.push(
      ["U", "R", "F", "L", "BL", "BR", "DR", "DL", "DBL", "B", "DBR", "D"][moves[i][0]] +
        ["", "2", "2'", "'"][moves[i][1]]
    );
  }
  return ret.join(" ");
...
function solveKiloCubie(cc: KiloCubie) {
  ...
  const sol1 = solve(
    ...
    function (idx: number[], move: number) {
      const idx1 = [doPhase1Move(idx[0], move), doPhase1Move(idx[1], y2Move[move])];
  ...
  const sol2 = solve(
    ...
    function (idx: number[], move: number) {
      const idx1 = [doPhase2Move(idx[0], move), doPhase2Move(idx[1], yMove[move])];
      if (idx1[0] == idx[0] && idx1[1] == idx[1]) {
        return null;
      }
      return idx1;
  ...
  const sol3 = solve(
    ...
    function (idx: number[]) {
      return Math.max(
        getPruning(Phase3Prun, idx[0]),
        getPruning(Phase3Prun, idx[1]),
        getPruning(Phase3Prun, idx[2])
      );
    },
    function (idx: number[], move: number) {
      return [

...
describe("EventLogger", () => {
  it("logs the complete typed event envelope to the visual event category", async () => {
    const factory = new TimerEventFactory({ now: () => 42.5 }, { next: () => "event-1" });
  ...
  it("also logs solve and generation events to the service category", async () => {
    ...
    const info = vi.fn();
    const eventLogger = new EventLogger(bus, { info });
    const event = factory.create(TIMER_EVENTS.SOLVES_LIST_REQUESTED, {
      ownerId: "timer:one",
  ...
  it("stops logging after destroy", async () => {
    const factory = new TimerEventFactory({ now: () => 1 }, { next: () => "event-1" });
    const bus = createApplicationEventBus(factory);
    const info = vi.fn();
    const eventLogger = new EventLogger(bus, { info });
    eventLogger.destroy();

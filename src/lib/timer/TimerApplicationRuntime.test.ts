...
describe("TimerApplicationRuntime", () => {
  ...
  it("composes one bus, factory, catalog, manager, and logger", async () => {
    ...
    expect(runtime.catalog.find(TIMER_DEVICE_IDS.KEYBOARD)?.managementMode).toBe(
      TIMER_DEVICE_MANAGEMENT_MODE.MANAGED
    );
    await runtime.bus.publish(
      runtime.events.create(TIMER_EVENTS.DEVICE_DISCOVERED, {

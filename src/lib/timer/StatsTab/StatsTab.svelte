  ...
  function updateChart(sv: Solve[]) {
    ...
    let avgSerie: echarts.SeriesOption[] = avgs.map((e, i) => {
      ...
      let data: any =
        pos > -1 && $STATS_WINDOW
          ? $STATS_WINDOW[pos]
          : getAverageS(e, sv, $session?.settings?.calcAoX || AverageSetting.SEQUENTIAL);
    ...
    timeChart.on("legendselectchanged", function (ev: any) {
      ...
      let hTrend: echarts.LineSeriesOption = allSeries.filter(
        s => s.name === "trend-high"
      )[0] as any;
    ...
    if (!stepTimeChart || stepTimeChart.isDisposed()) {
      ...
      let stepOption: echarts.EChartsOption = {
        ...
        tooltip: {
          ...tooltipStyle,
          valueFormatter(v) {
            return timer(+(v || "").toString(), true, true);
          },
        },
      };
...
<main class:headless class:multi={$session?.settings?.sessionType === "multi-step"} class="tx-text">
  ...
  <div class={"stats flex card " + (headless ? "max-md:hidden" : "")}>
    <StatsProgress
      title={$localLang.TIMER.best}
      pColor="green"
      label={timer($stats.best.value, true, true)}
      value={$stats.best.value}
      total={$stats.worst.value}
    />

    <StatsProgress
      title={$localLang.TIMER.worst}
      pColor="yellow"
    ...
    <StatsProgress
      title={$localLang.TIMER.count}
      label={$stats.count.value.toString()}
      hidebar
      value={$stats.count.value}
      total={$stats.count.value}
    />

    <StatsProgress
      title={$localLang.TIMER.totalTime}
      hidebar

  ...
  <div class="space-y-4">
    ...
    <div class="flex gap-2 flex-wrap">
      ...
      {#if selectedIds.size > 0}
        <Button onclick={onDeleteSelected} class="flex-1 btn-error">
        ...
        <tbody>
          {#each solves as solve (solve._id)}
            {#if editingId === solve._id}
              <tr class="bg-blue-50">
                <td>
                  <input type="checkbox" checked={selectedIds.has(solve._id!)} onchange={() => onToggleSelect(solve._id!)} />
                </td>
                ...
                <td>
                  <div class="flex gap-1">
                    <button class="btn btn-xs btn-success" onclick={saveEdit}>Save</button>
                    <button class="btn btn-xs btn-error" onclick={cancelEdit}>Cancel</button>
                  </div>
                </td>
              </tr>
            {:else}
              <tr>
                ...
                <td>
                  <span class={`badge ${solve.penalty === Penalty.DNF ? "badge-error" : solve.penalty === Penalty.P2 ? "badge-warning" : "badge-success"}`}>
                ...
                <td>
                  <div class="flex gap-1">
                    <button class="btn btn-xs btn-info" onclick={() => startEdit(solve)}>Edit</button>
                    <button class="btn btn-xs btn-error" onclick={() => onDelete(solve._id!)}>Delete</button>
                  </div>
                </td>
              </tr>
            {/if}

...
interface KeyboardMachineContext {
  ...
  onRunPaused(): void;
  onRunResumed(startedAt: number): void;
  onRunEnded(): void;
...
function inspectionDurationMs(context: KeyboardMachineContext): number {
  return (
    (context.view.session?.settings.inspection ||
...
const keyboardMachine = setup({
  ...
  delays: {
    ...
    inspectionP2: ({ context }) =>
      Math.max(0, context.inspectionStartedAt + inspectionDurationMs(context) - context.now()),
    inspectionDnf: ({ context }) =>
      Math.max(
        0,
  ...
  guards: {
    ...
    isPause: ({ event }) => event.code === KEYBOARD_CODES.PAUSE,
    ...
    hasInspectionAndSpace: ({ context, event }) =>
      event.code === KEYBOARD_CODES.SPACE &&
      (context.view.session?.settings.hasInspection ?? false),
  },
  actions: {
    ...
    publishReady: ({ context }) => {
      context.publish(
        context.events.create(TIMER_EVENTS.DEVICE_READY, {
    ...
    publishInspectionP2: ({ context }) => {
      context.publish(
        context.events.create(TIMER_EVENTS.DEVICE_PENALTY_APPLIED, {
          ownerId: context.ownerId,
          deviceId: TIMER_DEVICE_IDS.KEYBOARD,
          penalty: Penalty.P2,
          fromInspection: true,
    ...
    publishInspectionDnf: ({ context }) => {
      ...
      context.publish(
        context.events.create(TIMER_EVENTS.DEVICE_RUN_STOPPED, {
          ownerId: context.ownerId,
          deviceId: TIMER_DEVICE_IDS.KEYBOARD,
          elapsedMs: Infinity,
          steps: [],
    ...
    rememberPause: assign({
      pausedAt: ({ event }) => event.timestamp,
    }),
    rememberResume: assign({
      startedAt: ({ context, event }) =>
        context.startedAt + Math.max(0, event.timestamp - context.pausedAt),
      pausedAt: () => 0,
    }),
    publishPaused: ({ context, event }) => {
      context.onRunPaused();
      context.publish(
        context.events.fromNative(
          TIMER_EVENTS.DEVICE_PAUSED,
          { ownerId: context.ownerId, deviceId: TIMER_DEVICE_IDS.KEYBOARD },
          nativeTimestamp(event.timestamp)
        )
      );
    },
    publishResumed: ({ context, event }) => {
      context.onRunResumed(context.startedAt);
      context.publish(
        context.events.fromNative(
          TIMER_EVENTS.DEVICE_RESUMED,
          { ownerId: context.ownerId, deviceId: TIMER_DEVICE_IDS.KEYBOARD },
          nativeTimestamp(event.timestamp)
        )
      );
    ...
    inspection: {
      ...
      on: {
        ...
        KEY_UP: {
          guard: "isSpace",
          target: "running",
          actions: ["rememberStart", "publishStarted"],
        },
      },
    },
    running: {
      on: {
        KEY_DOWN: [
          { guard: "isPause", target: "paused", actions: ["rememberPause", "publishPaused"] },
          { guard: "isStopKey", target: "cooldown", actions: "publishStopped" },
        ],
      },
    },
    paused: {
      on: {
        KEY_DOWN: {
          guard: "isSpace",
          target: "running",
          actions: ["rememberResume", "publishResumed"],
        },
      },
    },
    cooldown: {
...
export class KeyboardDevice implements ITimerDevice {
  ...
  constructor(
    ...
    this.readingIntervalMs =
      options.readingIntervalMs ?? KEYBOARD_DEVICE_TIMING.DEFAULT_READING_INTERVAL_MS;
  ...
  start(context: TimerDeviceActivationContext): void {
    ...
    const actor = createActor(keyboardMachine, {
      input: {
        ...
        ownerId: context.ownerId,
        preventionMs: this.options.preventionMs ?? KEYBOARD_DEVICE_TIMING.DEFAULT_PREVENTION_MS,
        restartGapMs: this.options.restartGapMs ?? KEYBOARD_DEVICE_TIMING.DEFAULT_RESTART_GAP_MS,
        startedAt: 0,
        pausedAt: 0,
        inspectionStartedAt: 0,
        now: () => this.clock.now(),
        onInspectionStarted: (timestamp: number) =>
          this.startInspectionReadings(timestamp, context.readonlyView, context.onReading),
        onRunStarted: (timestamp: number) => this.startReadings(timestamp, context.onReading),
        onRunPaused: () => this.stopReadings(),
        onRunResumed: (startedAt: number) => this.startReadings(startedAt, context.onReading),
  ...
  private startInspectionReadings(
    ...
    this.stopReadings();
    const inspectionMs =
      (view.session?.settings.inspection ?? KEYBOARD_DEVICE_TIMING.DEFAULT_INSPECTION_SECONDS) *
      KEYBOARD_DEVICE_TIMING.MILLISECONDS_PER_SECOND;
    ...
    const update = () => {
      ...
      onReading({
        ...
        timeMs:
          Math.round(
            (inspectionEndsAt - currentTimestamp) / KEYBOARD_DEVICE_TIMING.MILLISECONDS_PER_SECOND
          ) * KEYBOARD_DEVICE_TIMING.MILLISECONDS_PER_SECOND,
        phase: "inspection",
      });
    };

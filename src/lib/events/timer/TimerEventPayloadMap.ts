import type { Penalty, Session, SessionSettings, Solve, Statistics, TimerState } from "@interfaces";
...
export interface TimerEventPayloadMap extends GenerationEventPayloadMap {
  ...
  [TIMER_EVENTS.DEVICE_STEP_COMPLETED]: OwnerDevicePayload & {
    stepNumber: number;
    elapsedMs: number;
  };
  [TIMER_EVENTS.DEVICE_PENALTY_APPLIED]: OwnerDevicePayload & {
    penalty: Penalty;
  ...
  [TIMER_EVENTS.SESSION_SETTINGS_CHANGE_REQUESTED]: {
    sessionId: string;
  ...
  [TIMER_EVENTS.NEW_RECORD]: {
    records: Array<{ name: string; previous: number; current: number }>;
  };

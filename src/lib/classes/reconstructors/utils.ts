...
export function centerStickerAligned(center: Piece, st: Sticker, ignoreColor = false): boolean {
  if (/^[xd]$/.test(st.color)) return false;

  const centerSticker = center.stickers.filter(nonGray)[0];
  const o = centerSticker.getOrientation();
...
export function pieceInPlace(centers: Piece[], piece: Piece): boolean {
  const edgeStickers = getColoredStickers(piece);
...
export function piecesCorrectRelative(pc1: Piece, pc2: Piece): boolean {
  const colors = getColoredStickers(pc1).map(st => st.color);
  ...
  for (let i = 0, maxi = colors.length; i < maxi; i += 1) {
    ...
    const v1 = pc1.stickers.filter(st => st.color === colors[i])[0].getOrientation();

    const v2 = pc2.stickers.filter(st => st.color === colors[i])[0].getOrientation();

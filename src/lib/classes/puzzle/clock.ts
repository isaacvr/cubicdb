...
export function CLOCK(): PuzzleInterface {
  ...
  const pieces = clock.pieces;
  const pinsArr: Piece[] = [];
  const dialsArr: Piece[] = [];
  const upFace = 0;
  ...
  function drawSingleClock(
    ...
    const levelSticker = (s: Sticker, layer: number, self = false) => {
    ...
    function pushExtrudedCircle(
      ...
      const c = circle(x, y, rad, col, layer);
      const c1 = new Piece([extrudeSticker(c.stickers[0], LAYER_V.mul(layer1 - layer))]);
    ...
    if (extrude) {
      ...
      const ang1 = -CLOCK_ANG * 0.96;
      const ang2 = CLOCK_ANG * 0.96;
      const a = UP.mul(RAD - BORDER1)
      ...
      const fst = extrudeSticker(st, LAYER_V.mul(CROSS_LAYER / 2), false, false);
      fst.color = WHITE;
    ...
    for (let i = -1; i < 2; i += 2) {
      for (let j = -1; j < 2; j += 2) {
        if (extrude) {
          // Wheel
          const wheelHandle = circle(0, 0, 0.1, BLACK, -CROSS_LAYER / 2);
          wheelHandle.stickers[0].points.forEach(p =>
            p.setCoords(X + W * i + p.x * 0.7, Y + W * j + p.y * 3, p.z)
          );
          wheelHandle.updateMassCenter();
          const wheel = extrudeSticker(
            wheelHandle.stickers[0],
            LAYER_V.mul(CROSS_LAYER),
            true,
            true
          );
    ...
    for (let i = -1; i < 2; i += 1) {
      for (let j = -1; j < 2; j += 1) {
        ...
        const arrowPiece = new Piece([levelSticker(arrow.add(ANCHOR), 10)]);

        const eArrowPiece = new Piece([
          extrudeSticker(arrowPiece.stickers[0], LAYER_V.mul(6 - 10)),
        ]);

        eArrowPiece.stickers[0].color = GRAY;

        const combinedArrow = new Piece([...arrowPiece.stickers, ...eArrowPiece.stickers]);
        ...
        for (let a = 0; a < 12; a += 1) {
          const pt = ANCHOR.add(
            UP.mul(RAD_CLOCK + BORDER / 2).rotate(CENTER, FRONT, (a * TAU) / 12)
          );
          const r = (circles.points[0].x / 4) * (a ? 1 : 1.6);
          const c = a ? WHITE : "red";
        ...
        if (BLACK === "black") {
          if (i <= 0 && j <= 0) {
            const px = Math.sign(ANCHOR.x + W / 2);
            const py = Math.sign(ANCHOR.y + W / 2);
            const pos = (px + 1) / 2 + 2 - (py + 1);
            const val = PINS[pos];

            const pinPiece = f(circle(ANCHOR.x + W / 2, ANCHOR.y + W / 2, R_PIN * 0.8, GRAY, 16));
            const pinPiece1 = f(
              circle(ANCHOR.x + W / 2, ANCHOR.y + W / 2, R_PIN * 0.8, BLACK, -139)
            );
            const pin = new Piece([
  ...
  function sortActor(arr: Piece[]) {
    arr.sort((p1, p2) => {
      const c1 = p1.getMassCenter();
      const c2 = p2.getMassCenter();
      const P1 = [c1.x, c1.y].map(Math.sign);
      const P2 = [c2.x, c2.y].map(Math.sign);
  ...
  const trySingleMove = (mv: any): PiecesToMove[] | null => {
    ...
    const res: PiecesToMove[] = [];

    sortActor(pinsArr);

    for (let i = 3; i >= 0; i -= 1) {
      const pinV = pinsArr[3 - i].getMassCenter().z > 0;
      const pin1V = !!(pinCode & (1 << i));
    ...
    for (let pos = 1; pos <= 2; pos += 1) {
      if (!isNaN(mv[pos])) {
        const dials = dialsArr.filter(d => d.stickers[0].points[0].z > 0);
        sortActor(dials);
        ...
        for (let i = 3; i >= 0; i -= 1) {
          ...
          if (val) {
            const rotationInfo = clock.toMove!(
              dials[3 - i],
  ...
  clock.move = function (moves: any[]) {
    for (let m = 0, maxm = moves.length; m < maxm; m += 1) {
      const mv = moves[m];
      const pcMoves = trySingleMove(mv);
  ...
  function getCoords(mc: Vector3D) {
    ...
    for (let i = -1; i <= 1; i += 1) {
      for (let j = -1; j <= 1; j += 1) {
        const dist = anchors[pos].sub(mc).abs();
        if (dist < minDist) {
          minDist = dist;
          coords = [i, j];
        }
  ...
  function getClosestDial(ct: Vector3D): Vector3D {
    ...
    for (let i = 0, maxi = anchors.length; i < maxi; i += 1) {
      const dist = ct.sub(anchors[i]).abs();
  ...
  clock.toMove = function (piece: Piece, sticker: Sticker, _dir: any, _pinCode: any) {
    ...
    if (sticker.name === "pin") {
      const val = piece.getMassCenter().z < 0;
    ...
    const coords = getCoords(mc);

    if (coords[0] === 0 || coords[1] === 0) return { pieces: [], ang: CLOCK_ANG };

    const pos = -(coords[1] - 1) + (coords[0] + 1) / 2;
    const arrows = pieces.filter(pc => pc.stickers.some(st => st.name === "arrow"));
    ...
    for (let i = 0; i < 4; i += 1) {
      if (getPinValue(i, _pinCode) === getPinValue(pos, _pinCode)) {
        const cd = getClocks(pinCoord[i]);
        arrowPieces = arrowPieces.concat(
          arrows.filter(pc => {
            const coord = getCoords(pc.stickers[0].getMassCenter());
            const side = Math.sign(pc.stickers[0].getMassCenter().z);
            const pinMult = getPinValue(pos, _pinCode) ? 1 : -1;
    ...
    return arrowPieces.map(p => {
      const ct = p.stickers.find(st => st.name === "arrow")!.updateMassCenter();
      const dial = getClosestDial(ct);
      const pcs = [p];

      if (Math.abs(dial.x) > EPS && Math.abs(dial.y) > EPS) {
        const wheels = pieces.filter(pc => pc.stickers.some(st => st.name === "wheel"));
        const coords = [Math.sign(dial.x), Math.sign(dial.y)];

        for (let i = 0, maxi = wheels.length; i < maxi; i += 1) {
          const wmc = wheels[i].getMassCenter();
          const wCoords = [Math.sign(wmc.x), Math.sign(wmc.y)];
  ...
  clock.applySequence = function (seq: string[]) {
    const moves = ScrambleParser.parseClock(seq.join(" "));

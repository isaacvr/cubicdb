...
const lens = [0, 0, 10, 0, 40, 60, 80, 100];
const modes = ["222so", "skbso", "pyrso", "333", "444wca", "555wca", "666wca", "777wca"];
const names = ["2x2", "Skewb", "Pyraminx", "3x3", "4x4", "5x5", "6x6", "7x7"];

export function identifyPuzzle(scramble: string): { mode: string; name: string; len: number } {
  const ini = [9, 8, 11, 16, 35, 60, 80, 100];
  const fin = [11, 11, 15, 22, 55, 62, 80, 100];
  ...
  const slen = scramble.split(/\s+/).length;
  let minIdx = 3;

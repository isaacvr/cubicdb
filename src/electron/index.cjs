...
let params = [prod ? appPath.replace(/app\.asar$/, "") : appPath, "src", "lib", "fixed"];
...
let server = https
  .createServer(
    {
      key: fs.readFileSync(join(dbPath, "key.pem")),
      cert: fs.readFileSync(join(dbPath, "cert.pem")),
    },
    eApp
  )
  .listen();
...
require(join(__dirname, "./serverHandlers/algorithms.cjs"))(ipcMain, Algorithms, dbPath);
require(join(__dirname, "./serverHandlers/tutorials.cjs"))(ipcMain, Tutorials, dbPath);
require(join(__dirname, "./serverHandlers/sessions.cjs"))(ipcMain, Sessions, Solves, dbPath);
require(join(__dirname, "./serverHandlers/solves.cjs"))(ipcMain, Solves, dbPath);
require(join(__dirname, "./serverHandlers/contests.cjs"))(ipcMain, Contests);
...
function createWindow() {
  ...
  if (serve || !prod) {
    ...
    //   electron: join(__dirname, '../node_modules', 'electron', 'build', 'electron.exe'),
    ...
    eApp.use(express.static(join(__dirname, "../../build")));
    eApp.get("*", (_, res) => {

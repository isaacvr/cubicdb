<script lang="ts">
  ...
  function getTutorial() {
    let map = new URL(($location as any).href).searchParams;
    ...
    dataService
      .getTutorial(parts[0], parts[1], lang)
      .then(t => {
        if (!t) {
          navigate("/tutorials");
        } else {
...
<Modal bind:open={showModal} outsideclose>
  {#if modalType === "delete"}
    <h2 class="text-center text-xl text-white">
      ¿Are you sure to delete step {tut.steps[sIndex].title}?
    </h2>
  ...
  {:else if modalType === "edit-tutorial"}
    ...
    <div class="flex flex-wrap gap-2 justify-center">
      <Button color="alternative" on:click={() => (showModal = false)}>Cancel</Button>
      <Button color="purple" on:click={saveTutorialConfig}>Save</Button>
    </div>
  {/if}
</Modal>

...
export function REX(): PuzzleInterface {
  ...
  let pieces = rex.pieces;

  let _ref1 = LEFT.add(BACK).add(UP);
  let _ref2 = LEFT.add(FRONT).add(UP);
  let _ref3 = RIGHT.add(BACK).add(UP);
  let _ref4 = RIGHT.add(FRONT).add(UP);

  let ref1 = UP.add(RIGHT.add(FRONT).mul(RAD));
  let ref2 = UP.add(RIGHT.add(BACK).mul(RAD));

  let A = new Vector3D(RAD - H, 1, 0);
  let B = A.rotate(CENTER, DOWN, PI_2);
  ...
  let piece1 = new Piece([sticker1, sticker1.rotate(_ref1, BACK, PI_2).reverse()]);
  let piece2 = new Piece([sticker2]);
  let piece3 = new Piece([sticker3]);
  ...
  rex.scramble = function () {
    ...
    for (let i = 0; i < MOVES; i += 1) {
      ...
      let pcs = rex.toMove(p, s, vec) as ToMoveResult;
      let cant = 1 + random(2);
